EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Interface:6822 U1
U 1 1 5DEB9E5F
P 2700 2650
F 0 "U1" H 2700 4231 50  0000 C CNN
F 1 "6822" H 2700 4140 50  0000 C CNN
F 2 "Package_DIP:DIP-40_W15.24mm" H 2750 1300 50  0001 L CNN
F 3 "http://pdf1.alldatasheet.com/datasheet-pdf/view/135452/MOTOROLA/MC6822.html" H 2700 2650 50  0001 C CNN
	1    2700 2650
	1    0    0    -1  
$EndComp
$Comp
L Interface:6822 U2
U 1 1 5DEBAE78
P 2700 5900
F 0 "U2" H 2700 7481 50  0000 C CNN
F 1 "6822" H 2700 7390 50  0000 C CNN
F 2 "Package_DIP:DIP-40_W15.24mm" H 2750 4550 50  0001 L CNN
F 3 "http://pdf1.alldatasheet.com/datasheet-pdf/view/135452/MOTOROLA/MC6822.html" H 2700 5900 50  0001 C CNN
	1    2700 5900
	1    0    0    -1  
$EndComp
Text Label 5250 1300 0    50   ~ 0
A0
Text Label 5250 1400 0    50   ~ 0
A1
Text Label 5250 1500 0    50   ~ 0
A2
Text Label 5250 1600 0    50   ~ 0
A3
Text Label 5250 1700 0    50   ~ 0
A4
Text Label 5250 1800 0    50   ~ 0
A5
Text Label 5250 1900 0    50   ~ 0
A6
Text Label 5250 2000 0    50   ~ 0
A7
Text Label 5250 2100 0    50   ~ 0
A8
Text Label 5250 2200 0    50   ~ 0
A9
Text Label 5250 2300 0    50   ~ 0
A10
Text Label 5250 2400 0    50   ~ 0
A11
Text Label 5250 2500 0    50   ~ 0
A12
Text Label 5250 2600 0    50   ~ 0
A13
Text Label 5250 2700 0    50   ~ 0
A14
Text Label 5250 2800 0    50   ~ 0
A15
Wire Wire Line
	4150 1900 3800 1900
Wire Wire Line
	4150 2400 3800 2400
Wire Wire Line
	4150 2500 3800 2500
Wire Wire Line
	4150 3000 3800 3000
Wire Wire Line
	4150 3100 3800 3100
Text Label 4000 1300 0    50   ~ 0
D0
Text Label 4000 1400 0    50   ~ 0
D1
Text Label 4000 1500 0    50   ~ 0
D2
Text Label 4000 1600 0    50   ~ 0
D3
Text Label 4000 1700 0    50   ~ 0
D4
Text Label 4000 1800 0    50   ~ 0
D5
Text Label 4000 1900 0    50   ~ 0
D6
Text Label 4000 2000 0    50   ~ 0
D7
Wire Wire Line
	2200 1450 1850 1450
Wire Wire Line
	2200 1550 1850 1550
Wire Wire Line
	2200 1650 1850 1650
Wire Wire Line
	2200 1750 1850 1750
Wire Wire Line
	2200 1850 1850 1850
Wire Wire Line
	2200 1950 1850 1950
Wire Wire Line
	2200 2050 1850 2050
Wire Wire Line
	2200 2150 1850 2150
Text Label 2050 1450 0    50   ~ 0
D0
Text Label 2050 1550 0    50   ~ 0
D1
Text Label 2050 1650 0    50   ~ 0
D2
Text Label 2050 1750 0    50   ~ 0
D3
Text Label 2050 1850 0    50   ~ 0
D4
Text Label 2050 1950 0    50   ~ 0
D5
Text Label 2050 2050 0    50   ~ 0
D6
Text Label 2050 2150 0    50   ~ 0
D7
Text Label 7400 1950 0    50   ~ 0
D0
Wire Wire Line
	7350 1950 7700 1950
Wire Wire Line
	7350 2050 7700 2050
Wire Wire Line
	7350 2150 7700 2150
Wire Wire Line
	7350 2250 7700 2250
Wire Wire Line
	7350 2350 7700 2350
Wire Wire Line
	7350 2450 7700 2450
Wire Wire Line
	7350 2550 7700 2550
Wire Wire Line
	7350 2650 7700 2650
Text Label 7400 2050 0    50   ~ 0
D1
Text Label 7400 2150 0    50   ~ 0
D2
Text Label 7400 2250 0    50   ~ 0
D3
Text Label 7400 2350 0    50   ~ 0
D4
Text Label 7400 2450 0    50   ~ 0
D5
Text Label 7400 2550 0    50   ~ 0
D6
Text Label 7400 2650 0    50   ~ 0
D7
Wire Wire Line
	6350 2700 6000 2700
Wire Wire Line
	6350 2600 6000 2600
Wire Wire Line
	6350 2500 6000 2500
Wire Wire Line
	6350 2400 6000 2400
Wire Wire Line
	6350 2300 6000 2300
Wire Wire Line
	6350 2200 6000 2200
Wire Wire Line
	6350 2100 6000 2100
Wire Wire Line
	6350 2000 6000 2000
Wire Wire Line
	6350 1900 6000 1900
Wire Wire Line
	6350 1800 6000 1800
Wire Wire Line
	6350 1700 6000 1700
Wire Wire Line
	6350 1600 6000 1600
Wire Wire Line
	6350 1500 6000 1500
Wire Wire Line
	6350 1400 6000 1400
Text Label 6150 2700 0    50   ~ 0
A14
Text Label 6150 2600 0    50   ~ 0
A13
Text Label 6150 2500 0    50   ~ 0
A12
Text Label 6150 2400 0    50   ~ 0
A11
Text Label 6150 2300 0    50   ~ 0
A10
Text Label 6150 2200 0    50   ~ 0
A9
Text Label 6150 2100 0    50   ~ 0
A8
Text Label 6150 2000 0    50   ~ 0
A7
Text Label 6150 1900 0    50   ~ 0
A6
Text Label 6150 1800 0    50   ~ 0
A5
Text Label 6150 1700 0    50   ~ 0
A4
Text Label 6150 1600 0    50   ~ 0
A3
Text Label 6150 1500 0    50   ~ 0
A2
Text Label 6150 1400 0    50   ~ 0
A1
Text Label 6150 1300 0    50   ~ 0
A0
Wire Wire Line
	6350 1300 6000 1300
$Comp
L B.E.M-sbc3x:TC55257PL-12 U4
U 1 1 5DEBFCD0
P 6850 1100
F 0 "U4" H 6850 1350 50  0000 C CNN
F 1 "TC55257PL-12" H 6850 1250 50  0000 C CNN
F 2 "Package_DIP:DIP-28_W15.24mm" H 6850 1100 50  0001 C CNN
F 3 "" H 6850 1100 50  0001 C CNN
	1    6850 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	8450 1250 8150 1250
Wire Wire Line
	8450 1350 8150 1350
Wire Wire Line
	8450 1450 8150 1450
Wire Wire Line
	8450 1550 8150 1550
Wire Wire Line
	8450 1650 8150 1650
Wire Wire Line
	8450 1750 8150 1750
Wire Wire Line
	8450 1850 8150 1850
Wire Wire Line
	8450 1950 8150 1950
Wire Wire Line
	8450 2050 8150 2050
Wire Wire Line
	8450 2150 8150 2150
Wire Wire Line
	8450 2250 8150 2250
Wire Wire Line
	8450 2350 8150 2350
Wire Wire Line
	8450 2450 8150 2450
Wire Wire Line
	8450 2550 8150 2550
Wire Wire Line
	8450 2650 8150 2650
Wire Wire Line
	8450 2850 8150 2850
Wire Wire Line
	8450 2950 8150 2950
Wire Wire Line
	8450 3050 8150 3050
Text Label 8250 1250 0    50   ~ 0
A0
Text Label 8250 1350 0    50   ~ 0
A1
Text Label 8250 1450 0    50   ~ 0
A2
Text Label 8250 1550 0    50   ~ 0
A3
Text Label 8250 1650 0    50   ~ 0
A4
Text Label 8250 1750 0    50   ~ 0
A5
Text Label 8250 1850 0    50   ~ 0
A6
Text Label 8250 1950 0    50   ~ 0
A7
Text Label 8250 2050 0    50   ~ 0
A8
Text Label 8250 2150 0    50   ~ 0
A9
Text Label 8250 2250 0    50   ~ 0
A10
Text Label 8250 2350 0    50   ~ 0
A11
Text Label 8250 2450 0    50   ~ 0
A12
Text Label 8250 2550 0    50   ~ 0
A13
Text Label 8250 2650 0    50   ~ 0
A14
Text Label 9300 1250 0    50   ~ 0
D0
Wire Wire Line
	9250 1950 9600 1950
Text Label 9300 1350 0    50   ~ 0
D1
Text Label 9300 1450 0    50   ~ 0
D2
Text Label 9300 1550 0    50   ~ 0
D3
Text Label 9300 1650 0    50   ~ 0
D4
Text Label 9300 1750 0    50   ~ 0
D5
Text Label 9300 1850 0    50   ~ 0
D6
Text Label 9300 1950 0    50   ~ 0
D7
Wire Wire Line
	2200 4700 1850 4700
Wire Wire Line
	2200 4800 1850 4800
Wire Wire Line
	2200 4900 1850 4900
Wire Wire Line
	2200 5000 1850 5000
Wire Wire Line
	2200 5100 1850 5100
Wire Wire Line
	2200 5200 1850 5200
Wire Wire Line
	2200 5300 1850 5300
Wire Wire Line
	2200 5400 1850 5400
$Comp
L B.E.M-sbc3x:DM74S571 U6
U 1 1 5DFDC735
P 6850 3400
F 0 "U6" H 6850 3631 50  0000 C CNN
F 1 "DM74S571" H 6850 3540 50  0000 C CNN
F 2 "Package_DIP:DIP-16_W7.62mm" H 6850 3400 50  0001 C CNN
F 3 "" H 6850 3400 50  0001 C CNN
	1    6850 3400
	1    0    0    -1  
$EndComp
Wire Wire Line
	9250 1850 9600 1850
Wire Wire Line
	9250 1750 9600 1750
Wire Wire Line
	9250 1650 9600 1650
Wire Wire Line
	9250 1550 9600 1550
Wire Wire Line
	9250 1450 9600 1450
Wire Wire Line
	9250 1350 9600 1350
Wire Wire Line
	9250 1250 9600 1250
$Comp
L Memory_EPROM:27256 U5
U 1 1 5DEBF046
P 8850 2150
F 0 "U5" H 8850 3431 50  0000 C CNN
F 1 "27256" H 8850 3340 50  0000 C CNN
F 2 "Package_DIP:DIP-28_W15.24mm" H 8850 2150 50  0001 C CNN
F 3 "http://datasheet.octopart.com/D27256-2-Intel-datasheet-17852618.pdf" H 8850 2150 50  0001 C CNN
	1    8850 2150
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 5E019E7D
P 8850 3250
F 0 "#PWR0101" H 8850 3000 50  0001 C CNN
F 1 "GND" H 8855 3077 50  0000 C CNN
F 2 "" H 8850 3250 50  0001 C CNN
F 3 "" H 8850 3250 50  0001 C CNN
	1    8850 3250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0102
U 1 1 5E01B18D
P 6850 2900
F 0 "#PWR0102" H 6850 2650 50  0001 C CNN
F 1 "GND" H 6855 2727 50  0000 C CNN
F 2 "" H 6850 2900 50  0001 C CNN
F 3 "" H 6850 2900 50  0001 C CNN
	1    6850 2900
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 5E01D852
P 6850 4550
F 0 "#PWR0103" H 6850 4300 50  0001 C CNN
F 1 "GND" H 6855 4377 50  0000 C CNN
F 2 "" H 6850 4550 50  0001 C CNN
F 3 "" H 6850 4550 50  0001 C CNN
	1    6850 4550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 5E01DE4F
P 4650 3900
F 0 "#PWR0104" H 4650 3650 50  0001 C CNN
F 1 "GND" H 4655 3727 50  0000 C CNN
F 2 "" H 4650 3900 50  0001 C CNN
F 3 "" H 4650 3900 50  0001 C CNN
	1    4650 3900
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0105
U 1 1 5E05C0C6
P 2700 7300
F 0 "#PWR0105" H 2700 7050 50  0001 C CNN
F 1 "GND" H 2705 7127 50  0000 C CNN
F 2 "" H 2700 7300 50  0001 C CNN
F 3 "" H 2700 7300 50  0001 C CNN
	1    2700 7300
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0106
U 1 1 5E05E804
P 2700 4500
F 0 "#PWR0106" H 2700 4350 50  0001 C CNN
F 1 "+5V" H 2715 4673 50  0000 C CNN
F 2 "" H 2700 4500 50  0001 C CNN
F 3 "" H 2700 4500 50  0001 C CNN
	1    2700 4500
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0107
U 1 1 5E0634E2
P 2700 1250
F 0 "#PWR0107" H 2700 1100 50  0001 C CNN
F 1 "+5V" H 2715 1423 50  0000 C CNN
F 2 "" H 2700 1250 50  0001 C CNN
F 3 "" H 2700 1250 50  0001 C CNN
	1    2700 1250
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0108
U 1 1 5E063A9E
P 4650 1100
F 0 "#PWR0108" H 4650 950 50  0001 C CNN
F 1 "+5V" H 4665 1273 50  0000 C CNN
F 2 "" H 4650 1100 50  0001 C CNN
F 3 "" H 4650 1100 50  0001 C CNN
	1    4650 1100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0109
U 1 1 5E0681F0
P 6850 1100
F 0 "#PWR0109" H 6850 950 50  0001 C CNN
F 1 "+5V" H 6865 1273 50  0000 C CNN
F 2 "" H 6850 1100 50  0001 C CNN
F 3 "" H 6850 1100 50  0001 C CNN
	1    6850 1100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0110
U 1 1 5E06862E
P 8850 1050
F 0 "#PWR0110" H 8850 900 50  0001 C CNN
F 1 "+5V" H 8865 1223 50  0000 C CNN
F 2 "" H 8850 1050 50  0001 C CNN
F 3 "" H 8850 1050 50  0001 C CNN
	1    8850 1050
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0111
U 1 1 5E068A88
P 6850 3350
F 0 "#PWR0111" H 6850 3200 50  0001 C CNN
F 1 "+5V" H 6865 3523 50  0000 C CNN
F 2 "" H 6850 3350 50  0001 C CNN
F 3 "" H 6850 3350 50  0001 C CNN
	1    6850 3350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0112
U 1 1 5E06E7E9
P 2700 4050
F 0 "#PWR0112" H 2700 3800 50  0001 C CNN
F 1 "GND" H 2705 3877 50  0000 C CNN
F 2 "" H 2700 4050 50  0001 C CNN
F 3 "" H 2700 4050 50  0001 C CNN
	1    2700 4050
	1    0    0    -1  
$EndComp
$Comp
L Connector:Screw_Terminal_01x02 J1
U 1 1 5E07743B
P 10750 850
F 0 "J1" H 10830 842 50  0000 L CNN
F 1 "Screw_Terminal_01x02" H 10830 751 50  0000 L CNN
F 2 "TerminalBlock:TerminalBlock_bornier-2_P5.08mm" H 10750 850 50  0001 C CNN
F 3 "~" H 10750 850 50  0001 C CNN
	1    10750 850 
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0113
U 1 1 5E088CFD
P 10550 950
F 0 "#PWR0113" H 10550 700 50  0001 C CNN
F 1 "GND" H 10555 777 50  0000 C CNN
F 2 "" H 10550 950 50  0001 C CNN
F 3 "" H 10550 950 50  0001 C CNN
	1    10550 950 
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0114
U 1 1 5E08A261
P 10550 850
F 0 "#PWR0114" H 10550 700 50  0001 C CNN
F 1 "+5V" H 10565 1023 50  0000 C CNN
F 2 "" H 10550 850 50  0001 C CNN
F 3 "" H 10550 850 50  0001 C CNN
	1    10550 850 
	1    0    0    -1  
$EndComp
Text Label 2000 4700 0    50   ~ 0
D0
Text Label 2000 4800 0    50   ~ 0
D1
Text Label 2000 4900 0    50   ~ 0
D2
Text Label 2000 5000 0    50   ~ 0
D3
Text Label 2000 5100 0    50   ~ 0
D4
Text Label 2000 5200 0    50   ~ 0
D5
Text Label 2000 5300 0    50   ~ 0
D6
Text Label 2000 5400 0    50   ~ 0
D7
Wire Wire Line
	5150 3500 5500 3500
Wire Wire Line
	5150 3400 5500 3400
Wire Wire Line
	5150 3200 5500 3200
Wire Wire Line
	5150 3100 5500 3100
Wire Wire Line
	5150 3000 5500 3000
Wire Wire Line
	5150 2800 5500 2800
Wire Wire Line
	5150 2700 5500 2700
Wire Wire Line
	5150 2600 5500 2600
Wire Wire Line
	5150 2500 5500 2500
Wire Wire Line
	5150 2400 5500 2400
Wire Wire Line
	5150 2300 5500 2300
Wire Wire Line
	5150 2200 5500 2200
Wire Wire Line
	5150 2100 5500 2100
Wire Wire Line
	5150 2000 5500 2000
Wire Wire Line
	5150 1900 5500 1900
Wire Wire Line
	5150 1800 5500 1800
Wire Wire Line
	5150 1700 5500 1700
Wire Wire Line
	5150 1600 5500 1600
Wire Wire Line
	5150 1500 5500 1500
Wire Wire Line
	5150 1400 5500 1400
Wire Wire Line
	5150 1300 5500 1300
Wire Wire Line
	4150 3400 3800 3400
Wire Wire Line
	4150 2300 3800 2300
Wire Wire Line
	4150 2000 3800 2000
Wire Wire Line
	4150 1800 3800 1800
Wire Wire Line
	4150 1700 3800 1700
Wire Wire Line
	4150 1600 3800 1600
Wire Wire Line
	4150 1500 3800 1500
Wire Wire Line
	4150 1400 3800 1400
Wire Wire Line
	4150 1300 3800 1300
$Comp
L CPU_NXP_6800:MC68A09 U3
U 1 1 5DEB9611
P 4650 2500
F 0 "U3" H 4650 4081 50  0000 C CNN
F 1 "MC68A09" H 4650 3990 50  0000 C CNN
F 2 "Package_DIP:DIP-40_W15.24mm" H 4650 1000 50  0001 C CNN
F 3 "http://pdf.datasheetcatalog.com/datasheet/motorola/MC68B09S.pdf" H 4650 2500 50  0001 C CNN
	1    4650 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 2200 3800 2200
Text Label 3850 2200 0    50   ~ 0
Reset
Wire Wire Line
	2200 3750 1850 3750
Text Label 1900 3750 0    50   ~ 0
Reset
Wire Wire Line
	2200 7000 1850 7000
Text Label 1900 7000 0    50   ~ 0
Reset
$EndSCHEMATC
